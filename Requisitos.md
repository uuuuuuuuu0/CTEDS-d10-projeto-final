Banco de dados: número do sorteio, resultado do sorteio, data do sorteio, nome, tipo, quantidade de ganhadores e seus nomes (a pensar)

(pensar sobre) Hospedar o banco na internet 

função que gere números aleatórios, e conferir quais são os modelos de sorteio

protótipo de baixa fidelidade e de alta fidelidade

Padrão de arquitetura em camadas (detalhar mais)

(a pensar) Exibição da quantidade de ganhadores, e seus nomes

(a pensar) Página de estatísticas com informações com: números mais jogados, múmeros mais sorteados, etc. Além disso, definir como vai ser o banco de dados nesse requisito


